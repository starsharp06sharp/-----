Module AVLTree
            ...
            If max IsNot node.leftTree Then
                ...
                If getHeight(parent.leftTree) - getHeight(parent.rightTree) > 1 Then
                    ...
                    If (getHeight(parent.leftTree.rightTree) > getHeight(parent.leftTree.leftTree)) Then
                        If parent Is grandParent.leftTree Then
                            grandParent.leftTree = LRRotation(parent)
                        Else
                            grandParent.rightTree = LRRotation(parent)
                        End If
                    Else
                        If parent Is grandParent.leftTree Then
                            grandParent.leftTree = LLRotation(parent)
                        Else
                            grandParent.rightTree = LLRotation(parent)
                        End If
                    End If
                    reCalcHeight(node)
                    If getHeight(node.rightTree) - getHeight(node.leftTree) > 1 Then
    ...
    Private Function RRRotation(ByRef a As BiTreeNode) As BiTreeNode
        ...
        b.leftTree = a
        a.height = max(getHeight(a.leftTree), getHeight(a.rightTree)) + 1
        b.height = max(a.height, getHeight(b.rightTree)) + 1
        Return b
    End Function
    ...
    Private Function reCalcHeight(ByRef node As BiTreeNode) As Integer
        Dim lh = -1
        Dim rh = -1
        If node.leftTree IsNot Nothing Then
            lh = reCalcHeight(node.leftTree)
        End If
        If node.rightTree IsNot Nothing Then
            rh = reCalcHeight(node.rightTree)
        End If
